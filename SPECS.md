...
While the AP is up, the lamp also runs a wildcard captive DNS responder on UDP `:53` that resolves every hostname to its own AP IP (`192.168.4.1`) and offers itself as the DHCP DNS server (option 6), plus a catch-all `HTTPD_404_NOT_FOUND` handler that 302-redirects any unmatched HTTP request — including the Apple/Android probe paths (`/hotspot-detect.html`, `/generate_204`) and Windows' NCSI probes (`/connecttest.txt`, `/ncsi.txt`) — to `/network`. Both are AP-mode only (the DNS task stops and the 404 handler falls through to a normal 404 once the lamp joins a home network), so joining the AP auto-opens the setup portal ("sign in to network") on macOS, iOS, Android, and Windows without the user having to type an address.
